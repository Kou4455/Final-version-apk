...
// User-provided CARTO Basemap API Key with environment variable override
const CARTO_API_KEY = ((import.meta as any).env?.VITE_CARTO_API_KEY as string | undefined) || 'cb1_2wi1_1_12536bda12b6917a62dbe090';
...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  return (
    <div 
      ...
      className={`relative w-full ${heightClass} bg-[#EFEAE2] rounded-3xl overflow-hidden select-none border border-[#E5DFD4] shadow-xs transition-all`}
    >
      ...
      <div className="absolute top-3 left-3 z-10 flex flex-wrap items-center gap-2 max-w-[85%]">
        <div className="px-3 py-1.5 bg-white/95 backdrop-blur-md rounded-full shadow-sm border border-[#E5DFD4] flex items-center gap-2 text-xs font-semibold text-[#181818]">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
          </span>
          <span className="truncate max-w-[140px] sm:max-w-[200px]">
            {mode === 'driver' ? 'Toto Partner GPS' : (activeRide?.status === 'in_progress' ? 'Trip in Progress' : 'Live E-Rickshaw Map')}
          </span>
          {currentAccuracy && (
      ...
      {tapHint && (
        <div className="absolute top-12 left-1/2 -translate-x-1/2 z-20 px-3 py-1 bg-[#181818] text-white text-xs font-bold rounded-full shadow-lg border border-white/20 animate-bounce">
          {tapHint}
        </div>
      )}

      {/* Bottom Left: Locate Me & Auto-Follow */}
      <div className="absolute bottom-3 left-3 z-10 flex items-center gap-2">
        <button
          type="button"
          onClick={handleCenterGpsClick}
          className="px-3 py-1.5 bg-white/95 backdrop-blur-md rounded-full shadow-sm border border-[#E5DFD4] flex items-center gap-1.5 text-xs font-semibold text-[#181818] hover:bg-white active:scale-95 transition-all cursor-pointer"
          title="Center on my live GPS location"
        >
          <LocateFixed className="w-3.5 h-3.5 text-[#FF6B2C]" />
          <span>Locate Me</span>
        </button>
        ...
        <button
          ...
          className={`px-2.5 py-1.5 rounded-full text-[11px] font-bold shadow-sm border transition-all cursor-pointer ${
            autoFollow 
              ? 'bg-emerald-50 text-emerald-700 border-emerald-200' 
              : 'bg-white/90 text-gray-600 border-[#E5DFD4]'
          }`}
          title={autoFollow ? 'Camera following vehicle/location' : 'Free map pan mode'}
        >
          {autoFollow ? 'Follow: ON' : 'Follow: OFF'}
        </button>
      </div>

      {/* Bottom Right Controls: Recenter + Zoom + Tile Style */}
